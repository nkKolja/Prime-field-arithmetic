...
void neg_tests() {
    const F zero = F::zero();
    const F pm1 = {}; pm1.data = F::pm1;

    // a + 0 = a
    for(int i = 0; i < TEST_LOOPS + 3; i++) {
        F a = (i >= 3) ? F::random() : (i % 3 == 0) ? zero : (i % 3 == 1) ? pm1 : F::random();
    ...
    // (a + b) + c = a + (b + c)
    for(int i = 0; i < TEST_LOOPS + (3 * 3 * 3); i++) {
        F a = (i >= 27) ? F::random() : (i % 3 == 0) ? zero : (i % 3 == 1) ? pm1 : F::random();
        F b = (i >= 27) ? F::random() : ((i/3) % 3 == 0) ? zero : ((i/3) % 3 == 1) ? pm1 : F::random();
        F c = (i >= 27) ? F::random() : ((i/(3*3)) % 3 == 0) ? zero : ((i/(3*3)) % 3 == 1) ? pm1 : F::random();
